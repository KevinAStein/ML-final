...
def setup(agent):
    ...
    try:
        agent.model.load_state_dict(torch.load('agent_code/run_and_collect_075/AC_geheimagent.pth'))
        agent.model.eval()
        print('model loaded')
...
def end_of_episode(agent):
    ...
    torch.save(agent.model.state_dict(), 'agent_code/run_and_collect_075/AC_geheimagent.pth') 
    print('model saved')
    agent.states = []
    agent.actions_done = []
    agent.reward_received = []
