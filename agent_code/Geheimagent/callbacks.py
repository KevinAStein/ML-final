...
def setup(agent):
    print('Awarding license to kill....')
    agent.reward = 0
    ...
    agent.model2 = ActorCritic(agent.INPUT_SHAPE)
    agent.model3 = ActorCritic(agent.INPUT_SHAPE)
    agent.model4 = ActorCritic(agent.INPUT_SHAPE)
    ...
    try:
        agent.model.load_state_dict(torch.load('agent_code/Geheimagent/Geheimagent_1.pth'))
        agent.model2.load_state_dict(torch.load('agent_code/Geheimagent/Geheimagent_2.pth'))
        agent.model3.load_state_dict(torch.load('agent_code/Geheimagent/Geheimagent_3.pth'))
        agent.model4.load_state_dict(torch.load('agent_code/Geheimagent/Geheimagent_4.pth'))
        agent.model.eval()
        agent.model2.eval()
        agent.model3.eval()
        agent.model4.eval()
...
def act(agent):
    ...
    probs_collection.append(action_probs.detach().numpy().flatten())


    state_value = agent.model2.get_state_value(state)
    action_probs = agent.model2.get_action_probs(state)
    probs_collection.append(action_probs.detach().numpy().flatten())

    state_value = agent.model3.get_state_value(state)
    action_probs = agent.model3.get_action_probs(state)
    probs_collection.append(action_probs.detach().numpy().flatten())

    state_value = agent.model4.get_state_value(state)
    action_probs = agent.model4.get_action_probs(state)
    probs_collection.append(action_probs.detach().numpy().flatten())

    probs_collection = np.asarray(probs_collection)
    #print(probs_collection)
    #print(np.mean(probs_collection, axis = 0))
    #print(np.argmax(np.mean(probs_collection, axis = 1)))
    idx = np.argmax(np.mean(probs_collection, axis = 0))
...
def end_of_episode(agent):
    ...
    agent.episode_durations.append(agent.game_state['step'] * 10)
